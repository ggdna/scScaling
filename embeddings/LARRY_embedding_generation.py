...
run_nanotx_every = 3  # run nanoTx every x iterations
dataset_fractions = np.logspace(-3, 0, 10)
...
# load data
ad = sc.read_h5ad(f'../../scaling_playground/data/LARRY_Q{qual:.3f}_rep{r}.h5ad')
UMI_per_cell = ad.raw.X.sum() / len(ad)

# split data into seen and held-out
seen, held_out = split_ad(ad, frac=0.5)

# create subfolder for this size/quality
output_dir = f'LARRY/Q{qual:.3f}_rep{r}/'
